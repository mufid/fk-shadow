...
public class Navigator
{
    ...
    public void determineHeadingChangeToGoToPoint( Point2D destination )
    {
...
//        double testY = this.robot.getY() + Utils.getCartesianY(WALL_STICK, rightTurnRadians );
        ...
        this.lastRightTurnRadians = rightTurnRadians +Math.signum(generatRandomPositiveNegitiveValue(1, -1)) * Math.random();
        this.robot.setTurnRightRadians( this.lastRightTurnRadians );
    }
    ...
    public void processEvent2( HitRobotEvent event )
    {
    	
    	double turnGunAmt = -Utils.normalRelativeAngle( event.getBearing() + this.robot.getHeading() - this.robot.getGunHeading() );
    	this.robot.setTurnRight(turnGunAmt);
    	this.robot.ahead(100);
//        System.out.println( "hit Friend dr " );
//        if ( this.robot.getDistanceRemaining() == 0.0D )
//        {
//            this.robot.setTurnRight(Utils.normalRelativeAngle(event.getBearing()));
//            double forwardValue = 1000+185.0D * Math.random() *this.FORWARD;
//            this.robot.setAhead( forwardValue );
//            System.out.println(event.getBearing()+" "+forwardValue);
//        }
    }
